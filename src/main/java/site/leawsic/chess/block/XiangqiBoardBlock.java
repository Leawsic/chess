...
public class XiangqiBoardBlock extends BlockWithEntity {
    ...
    @Override public BlockState getPlacementState(ItemPlacementContext ctx) { return BoardMultiblock.canPlace(ctx.getWorld(), ctx.getBlockPos()) ? getDefaultState() : null; }
    @Override public void onPlaced(World world, BlockPos pos, BlockState state, net.minecraft.entity.LivingEntity placer, net.minecraft.item.ItemStack stack) { super.onPlaced(world, pos, state, placer, stack); if (!world.isClient) BoardMultiblock.placePlaceholders(world, pos, offset -> ModBlocks.XIANGQI_PLACEHOLDER.getDefaultState().with(BoardPlaceholderBlock.OFFSET_X, offset.getX() + 1).with(BoardPlaceholderBlock.OFFSET_Z, offset.getZ() + 1)); }
